...
class MembreController extends Controller
{
    public function indexAction(Request $request) {
        ...
        if($request->isMethod('POST')) {
            ...
            $sql = "SELECT DISTINCT(titre), contenu, datePublication,photo,type, auteur    FROM article_noucoze WHERE category_id='".$category."'";
    ...
    public function confirmerAction(Request $request, $id) {
        ...
        return $this->redirect($this->generateUrl('liste', array('user'=>$username)));
    ...
    public function ajouterAction($id, Request $request) {
        ...
        return $this->redirect($this->generateUrl('invite' , array('user'=>$username)));
			...
			->getScalarResult();
             ...
             else
             {
                ...
                //$articles = $em->getRepository('WebFrontBundle:Articles')->findBy(array('country'=>$countryId));
                 ...
                 $rsm = new ResultSetMapping();
                 $sql = "SELECT DISTINCT(titre), contenu, datePublication,photo,type,auteur    FROM article_noucoze";
    ...
    public function posterAction($user, Request $request){
            ...
            $auteur = $request->getSession()->get('user');
    ...
    public function etatArticleAction($user, $id, Request $request){
        
        
        
        if ($request->get('etat_public')) {
            $etat = $request->get('etat_public');
        }
        elseif ($request->get('etat_amis')){
            $etat = $request->get('etat_amis');
        }
        elseif ($request->get('etat_moi')){
            $etat = $request->get('etat_moi');
        }
        
        $conn = $this->get('database_connection');
        $update = $conn->executeUpdate("UPDATE article_noucoze  SET etat = '".$etat."' WHERE id = '".$id."'");
        ...
        $articles = $em->getRepository('WebFrontBundle:Articles')->findBy(array('country'=>$countryId));
        $categories = $em->getRepository('WebFrontBundle:Categorie')->findAll();
        $liste = $em->getRepository('WebFrontBundle:Articles')->findBy(array('publication' => $user));
        return $this->render('WebFrontBundle:Membre:liste_article_etat.html.twig', 
                array(
                    'countries' => $countries,
                    'articles' => $articles,
                    'liste' => $liste,
    ...
    public function mesArticleAction($user, Request $request, $etat = 'moi'){
        $countryId = (integer) $request->query->get('country',1);
        $em = $this->getDoctrine()->getManager();
        $countries = $em->getRepository('WebFrontBundle:Country')->findAll();
        $categories = $em->getRepository('WebFrontBundle:Categorie')->findAll();
        ...
        $sql = "SELECT DISTINCT(titre), contenu, datePublication,photo,type,auteur    FROM article_noucoze WHERE
        publication = '".$user."' AND etat = '".$etat."'";
        ...
        $rsm->addScalarResult('contenu', 'contenu');
        $articles = $em->createNativeQuery($sql, $rsm)->getScalarResult();
        
        return $this->render('WebFrontBundle:Membre:mes_articles.html.twig',
         array(   
            'categories' => $categories,
            'articles' => $articles,
            'countries' => $countries
    ...
    public function sharedArticlesAction(Request $request) {
        ...
        $categories = $em->getRepository('WebFrontBundle:Categorie')->findAll();
        $user = $em->getRepository('AppCoreBundle:Utilisateur')->findOneBy(array('username' => $username));
        $friends = $em->getRepository('WebFrontBundle:Invitation')->findBy(array('utilisateur' => $user, 'etat' => 1));

        $sharedArticles = array();

        foreach( $friends as $friend ){
            /*$articles = $em->createQuery('SELECT a FROM WebFrontBundle:Articles a WHERE a.auteur = :username AND a.etat = :etat')
                ->setParameter('username', $friend->getAmi()->getUsername())
                ->setParameter('etat', 'amis')
                ->getResult();
            */
            $articles = $em->getRepository('WebFrontBundle:Articles')->findBy(
                array(
                    'auteur'=>$friend->getAmi()->getUsername(),
                    'etat'=>'amis'
                ));
            if($articles) {
        ...
        return $this->render('WebFrontBundle:Membre:sharedArticles.html.twig',
            array(
                'categories' => $categories,
